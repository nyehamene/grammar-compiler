...
func (s *Server) handleTextDocumentFormatting(id int, msg map[string]any) {
	...
	if p, ok := msg["params"]; ok {
		...
		if err != nil {
			s.sendErrorResponse(id, InternalError, fmt.Sprintf("Failed to marshal params: %v", err))
			return
		}
		if err := json.Unmarshal(encodedParams, &params); err != nil {
			s.sendErrorResponse(id, InternalError, fmt.Sprintf("Failed to unmarshal params: %v", err))
			return
		}
	} else {
		s.sendErrorResponse(id, InvalidParams, "missing params for textDocument/formatting")
		return
	}

	content, ok := s.GetDocumentContent(params.TextDocument.URI)
	if !ok {
		s.sendErrorResponse(id, InternalError, fmt.Sprintf("document not found: %s", params.TextDocument.URI))
		return
	}
	...
	if err != nil {
		...
		s.log.Printf("error formatting document %s: %v", params.TextDocument.URI, err)
		s.sendResponse(id, nil, nil) // No edits
		return
	}
